  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4" onClick={onCancel}>
      <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <h3 className="text-base font-bold text-slate-900">Delete upload?</h3>
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 py-8">
      <div className="mx-auto w-full max-w-2xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-6">
          <h1 className="text-xl font-bold text-[#056559]">AI Bone Age Analysis</h1>
          <p className="mt-1 text-sm text-slate-500">
            For parents who <span className="font-medium text-slate-900">already have a hand X-ray</span> and
        ...
        {!model.ready && (
          <div className="mb-4 flex gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        {uploadError && (
          <div className="mb-4 flex items-start justify-between gap-3 rounded-2xl border border-red-200 bg-red-50 p-4">
        ...
        <div
          ...
          className={`mb-4 flex cursor-pointer flex-col items-center rounded-2xl border-2 border-dashed bg-white p-8 text-center transition ${
            dragActive ? 'border-[#056559] bg-[#f2fbf9]' : 'border-slate-200 hover:border-slate-300'
          }`}
        >
          <UploadCloud size={28} className="mb-2 text-slate-400" />
          <p className="font-semibold text-slate-900">Drag and drop X-ray image here</p>
          <p className="mb-4 text-xs text-slate-500">Supports JPEG or PNG, max 10MB</p>
          ...
          <button
            ...
            className="rounded-full bg-[#056559] px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-[#03443c]"
          >
            Browse Files
          </button>
        ...
        <div className="rounded-2xl bg-white p-5 border border-slate-200 shadow-2xs">
          ...
          <div className="flex flex-col gap-3">
            {history.map((p) => (
              <div key={p.id} className="flex items-center gap-3 border-b border-slate-100 pb-3 text-sm last:border-0 last:pb-0">
                <img
                  src={p.previewUrl}
                  alt="X-ray upload"
                ...
                <div className="flex-1">
                  <p className="text-slate-500">{formatDate(p.createdAt)}</p>
                  <p className="font-medium text-slate-500">Analysing…</p>
                </div>
            ...
            {history.length === 0 && <p className="text-sm text-slate-500">No uploads yet.</p>}
          </div>
        </div>
